...
/*
  * product names are generated by: brand, category, subcategory, type, power, model, standard, SKU
  */
export const createProductName = (
  ...
  const systemSize =
    systemSizeID && systemSizeID.label !== 'N/A'
      ? `: ${systemSizeID.label}`
      : '';
  const SKU = SKUinput && SKUinput.length ? `: ${SKUinput}` : '';
  return `${brand}${category}${subcategory}${productType}${power}${systemSize}${SKU}`;
};
