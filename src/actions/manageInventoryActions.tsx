...
/*
  * product names are generated by: SKU (optional), brand, category, subcategory, type, power, model, standard
  */
export const createProductName = (
  ...
  const systemSize =
    systemSizeID && systemSizeID.label !== 'N/A'
      ...
      : '';
  if (SKU.length) {
    return `${SKU}${brand}${category}${subcategory}${productType}${power}${systemSize}`;
  }
  return `${brand}${category}${subcategory}${productType}${power}${systemSize}`;
};
